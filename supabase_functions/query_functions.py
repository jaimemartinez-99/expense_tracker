...
from telegram_functions.telegram_send_functions import send_message
...
def save_expense(text, supabase_client, chat_id):
    try:
        ...
        if len(parts) != 5:
            error_message = "❌ Formato incorrecto. Usa: /savexpense tipo total fecha (ej. /savexpense comida 12.50 21072025)"
            print(error_message)
...
def save_entry(text, supabase_client, chat_id):
    try:
        parts = text.strip().split()

        if len(parts) != 5:
            error_message = "❌ Formato incorrecto. Usa: /saventry tipo total fecha (ej. /saventry comida 12.50 21072025)"
            print(error_message)
            send_message(chat_id, error_message)
            return
        ...
        response = supabase_client.table('ingresos').insert({
            'tipo': tipo,
            'detalle': detalle,
            'total': total,
            'created_at': fecha.isoformat()
        ...
        if response.data:
            confirmation = f"✅ Beneficio guardado:\nTipo: {tipo}\nDetalle: {detalle}\nTotal: {total:.2f}€\nFecha: {fecha.strftime('%d/%m/%Y')}"
            print(confirmation)
            send_message(chat_id, confirmation)
            return True
        else:
            error_message = f"❌ Error guardando beneficio. Respuesta: {response}"
            print(error_message)
            send_message(chat_id, error_message)

    except Exception as e:
        error_message = f"❌ Error inesperado en save_entry: {e}"
        print(error_message)
        send_message(chat_id, error_message)
